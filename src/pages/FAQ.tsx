...
export const FAQ: React.FC = () => {
  ...
  const faqItems = [
    ...
    },
    {
      ...
      a: `每一件产品在 PM 全球系统里都设定了对应数值的系统积分。以常见“自动购”情景为例：基础套自动购约 ${autoBuyPrice} 元覆盖约3个月，奖金核算通常按“月度积分”折算；本站演示以每月约 ${points} 积分作为一位持续自动购成员的参考起点。计算奖金时，是以当月团队产生的积分总和为基准，先乘以“奖金换算系数（如0.51）”，再乘以对应代数提成比（如第一代5%），最后乘以结算币种的“演示兑换汇率（如8.2）”转换得出最终的人民币金额。积分是全球系统统一计价的桥梁。`,
    },
    {
      id: 'faq-6',
    ...
    },
    {
      ...
      a: `常见原因是“口径不同”。有的资料按某个国家/渠道的标定方式展示，有的资料把自动购按月折算后展示，也可能混用了不同市场版本。本站目前采用“自动购按月折算”的演示口径（每月约 ${points} 积分），并明确标注为情景测算参数。正式使用前，请以您注册地后台当月结算清单为准。`,
    },
  ];
